...
a.annotate("diffusivite\ntypique ~0.55",(0.55,rms_a(0.55)),textcoords="offset points",
           xytext=(-60,30),fontsize=8,color="gray")
a.set_xlabel("diffusivite de surface a (fm)"); a.set_ylabel("rms sur la peau (fm)")
...
a.bar(x-w,pm,width=w,color="k",label="mesure")
...
a.set_title("B -- Peau : sharp (P9) ~ 0, diffus reproduit\nle basculement Ca < Pb est retrouve",fontsize=9.5)
a.legend(fontsize=8); a.grid(alpha=0.3,axis="y")

fig.suptitle("P19 -- Borne quantitative sur la surface diffuse (l'echec P9/P11 mesure)",fontsize=12)
