...
AC_ALPHA = 0.54    # MeV, pour Qalpha (recale sur 238U=4.27 mesure)
R0FM = 1.2
...
def Qalpha(A, Z):  # energie liberee par emission alpha
...
leg=[mp.Patch(color=cmap(i),label=l) for i,l in enumerate(["stable","beta-","beta+","alpha","fission"])]
ax.legend(handles=leg,fontsize=9,loc="upper left")
ax.set_title(f"P16 -- Carte unifiee des modes de desintegration (rouge = desaccord)\nscore {acc}/{len(rows)} nucleides de reference",fontsize=11)
