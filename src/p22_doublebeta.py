...
json.dump(out, open(os.path.join(_DATA_DIR, "p22_doublebeta.json"), "w"), indent=2, ensure_ascii=False)
...
plt.tight_layout()
plt.savefig(os.path.join(_DATA_DIR, "p22_doublebeta.png"), dpi=110)
for pth in ["p22_doublebeta.py", "p22_doublebeta.json", "p22_doublebeta.png"]:
    h = hashlib.sha256(open(os.path.join(_DATA_DIR, pth), 'rb').read()).hexdigest()[:12]
    open(os.path.join(_DATA_DIR, f"sha_{pth.split('.')[0]}_{pth.split('.')[1]}.txt"), "w").write(h)
    print(pth, h)
