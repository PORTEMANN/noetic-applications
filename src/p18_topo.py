...
#   protege par la topologie (phase non triviale) vs absence (phase triviale).
...
a.set_title("B -- Chern des bandes = (1, -2, 1)\nHall : sigma = C e^2/h quantifiee",fontsize=9.5)
...
a.annotate(f"{zm_topo} zero-modes\n(bords)",(0,0),textcoords="offset points",xytext=(15,-10),fontsize=9,color="seagreen")
a.set_xlabel("E (t)"); a.set_yticks([0,1]); a.set_yticklabels(["topo","triviale"])
a.set_title("C -- SSH : etats de bord proteges\n2 zero-modes (topo) vs 0 (triviale)",fontsize=9.5)
a.legend(fontsize=8,loc="upper right")

fig.suptitle("P18 -- Les etats topologiques de la matiere : Chern (Hall) et bords (SSH)",fontsize=12)
