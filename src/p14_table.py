...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")  # output to repo root
ALPHA = 1.0/137.036
A0    = 137.036
MU    = 207.0
R0FM  = 1.2
...
a=ax[0]
a.semilogy(Z,np.abs(de),"o-",color="steelblue",ms=3,label=r"$|\delta_{1s}|$ electron")
...
a.set_xlabel("Z"); a.set_ylabel(r"signature coeur fini $|\delta_{1s}|$")
a.set_title("A -- Signature coeur fini : 10-4 (H) -> ~1 (Pb)\nle muon amplifie la sensibilite",fontsize=10)
a.legend(fontsize=9); a.grid(alpha=0.3,which="both")
a=ax[1]
a.set_yscale("log")
a.plot(Z,re_,"o-",color="seagreen",ms=3,label=r"$R_c/a_{1s}$ electron")
a.plot(Z,rm,"s-",color="darkorange",ms=3,label=r"$R_c/a_{1s}$ muon")
a.axhline(1,color="k",ls="--",lw=1)
a.axvline(ze,color="seagreen",ls=":",lw=1)
a.annotate(f"electron Rc/a1s=1\nZ={ze} ({[r['sym'] for r in rows if r['Z']==ze][0]})",(ze,1.4),fontsize=8,color="seagreen")
a.annotate("muon sature\ndes Z=1",(3,300),fontsize=8,color="darkorange")
a.set_xlabel("Z"); a.set_ylabel(r"$R_c/a_{1s}$ (log)")
a.set_title("B -- Frontiere : l'electron devient sensible au coeur\nvers Z=12 (Mg), le muon y est toujours",fontsize=10)
a.legend(fontsize=9,loc="upper left"); a.grid(alpha=0.3,which="both")
fig.suptitle("P14 -- Table des cartes d'identite atomiques H->U (operateur radial a coeur fini)",fontsize=12)
