...
json.dump(out, open(os.path.join(_DATA_DIR, "p21_polarite.json"), "w"), indent=2, ensure_ascii=False)
...
plt.tight_layout()
plt.savefig(os.path.join(_DATA_DIR, "p21_polarite.png"), dpi=110)
for p in ["p21_polarite.py", "p21_polarite.json", "p21_polarite.png"]:
    h = hashlib.sha256(open(os.path.join(_DATA_DIR, p), 'rb').read()).hexdigest()[:12]
    open(os.path.join(_DATA_DIR, f"sha_{p.split('.')[0]}_{p.split('.')[1]}.txt"), "w").write(h)
    print(p, h)
