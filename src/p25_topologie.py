...
fig.tight_layout()
fig.savefig(os.path.join(_DATA_DIR, "p25_topologie.png"), dpi=140)

with open(os.path.join(_DATA_DIR, "p25_topologie.json"), "w") as f:
    json.dump(out, f, indent=2, ensure_ascii=False)
def sha(path):
    return hashlib.sha256(open(path, "rb").read()).hexdigest()[:12]
for fn in ["p25_topologie.py", "p25_topologie.json", "p25_topologie.png"]:
    print(fn, sha(os.path.join(_DATA_DIR, fn)))
