...
a=ax[0]
a.plot(xr,yr,"o",color="steelblue",ms=9,label="rho radiale (n)")
a.plot(xo,yo,"s",color="indianred",ms=9,label="rho/a orbitale (L=J)")
...
a.plot(xg,pente_univ*xg+0.5,":",color="k",lw=1.4,label=f"1/(2pi sigma) = {pente_univ:.2f}")
for (n,m),tag in zip(RHO_RAD,["rho","rho(1450)","rho(1700)","rho(2150)"]):
    a.annotate(tag,(m*m,n),textcoords="offset points",xytext=(5,-11),fontsize=7.5)
for (L,m),tag in zip(RHO_ORB,["rho","a2","rho3","a4"]):
    a.annotate(tag,(m*m,L),textcoords="offset points",xytext=(5,4),fontsize=7.5)
a.set_xlabel(r"$M^2$ (GeV$^2$)"); a.set_ylabel("n  ou  J")
a.set_title(f"A -- Regge : pente universelle 1/(2pi sigma)\nradiale {slope_rad:.2f} | orbitale {slope_orb:.2f} | theorie {pente_univ:.2f} GeV$^{{-2}}$",fontsize=9.5)
...
x=np.arange(4)
a.plot(x,mes,"o",color="seagreen",ms=11,label="ecarts mesures",zorder=3)
a.plot(x,calc_n,"s",color="darkorange",ms=9,label="ecarts calcules (Cornell)",zorder=3)
...
a.set_ylabel("ecart depuis 1S (MeV)")
a.set_title(f"B -- Charmonium (structure des ecarts)\nS-P {ecarts['SP_calc_MeV']:.0f} vs {ecarts['SP_mes_MeV']} | 2S-1S {ecarts['S21_calc_MeV']:.0f} vs {ecarts['S21_mes_MeV']} MeV",fontsize=9.5)
a.legend(fontsize=9); a.grid(alpha=0.3)

fig.suptitle("P15 -- Le spectre mesure des hadrons : Regge (lineaire) + charmonium (Cornell)",fontsize=12)
